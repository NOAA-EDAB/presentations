...
.pull-left[
Drivers:
* decline in commercial landings is most likely driven by the requirement to rebuild individual stocks (lower quotas) as well as market dynamics
...
```{r , fig.asp=0.3, fig.alt="Fish community functional traits in New England based on Fall (red) and Spring (blue) survey data. Length at maturity for the full finfish community has increased in spring (orange line), but decreased in fall (purple lines)"}
...
.pull-left[
```{r ,  fig.width = 7, fig.asp = 1, results='hide'}
...
```{r , fig.alt= "Recreational engagement and population relative engagement with labels for the top recreationally engaged fishing communities in the Mid-Atlantic.", fig.width = 7, fig.asp = 1, results='hide'}
...
---
## Risks to Spatial Management: All Areas
...
```{r , fig.alt = "Index representing changes in the location of the Gulf Stream north wall (black). Positive values represent a more northerly Gulf Stream position, with increasing trend (orange)."}
...
```{r , fig.alt="Seasonal cold pool mean temperature (left) and spatial extent index (right), based on bias-corrected ROMS-NWA (open circles) and GLORYS (closed circles), with declining trends (purple).", fig.width = 7.5, fig.asp = 0.4, results='hide'}
...
```{r , fig.width=14, fig.height=8, fig.alt="Monthly median chlorophyll a concentration in the MAB (black) with significant increase in January (orange line) and decrease in September (purple line)."}
