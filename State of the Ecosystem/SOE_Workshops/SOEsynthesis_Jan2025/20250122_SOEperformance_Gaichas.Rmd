...
p <- commsocvulrank |>
  ...
  ggplot2::guides(color=guide_legend(title="EJ Vulnerability Rank"))+
  ggrepel::geom_label_repel(aes(label = ifelse(ComEng>2,as.character(GEO_NAME),ifelse(ComRel>1.06,as.character(GEO_NAME),''))),
                   size=2,
                   box.padding   = 0.3, 
                   point.padding = 0.1,
...
p <- commsocvulrank |>
  dplyr::filter(STATEABBR %in% c("DE", "MD", "NJ", "NY", "NC", "PA", "VA")) |>
  ggplot2::ggplot(ggplot2::aes(x=RecEng, y=RecRel)) + 
  ggplot2::geom_point(shape=1, 
             size=1.4, stroke=0.9, alpha=0.6, 
             aes(color=as.factor(EJrank)))+
  ggplot2::scale_colour_viridis_d(direction = -1) +
...
p <- commsocvulrank |>
  dplyr::filter(STATEABBR %in% c("CT", "ME", "MA", "NH", "RI")) |>
  ggplot2::ggplot(ggplot2::aes(x=ComEng, y=ComRel)) + 
  ggplot2::geom_point(shape=1, 
             size=1.4, stroke=0.9, alpha=0.6, 
             aes(color=as.factor(EJrank)))+
...
p <- commsocvulrank |>
  ...
  ggplot2::xlab(expression(paste("Commercial Engagement Index", italic(" (factor score)"))))+
  ggplot2::guides(color=guide_legend(title="EJ Vulnerability Rank"))+
  ggrepel::geom_label_repel(aes(label = ifelse(RecEng>2.4,as.character(GEO_NAME),ifelse(RecRel>1,as.character(GEO_NAME),''))),
                   size=2,
                   box.padding   = 0.3, 
                   point.padding = 0.1,
                   segment.color = 'grey50',
