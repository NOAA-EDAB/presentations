...
author: Sarah Gaichas$^{1,2}$ <br /> <br /> Thanks to Geret DePiper$^{1,2}$, Brandon Muffley$^3$
...
* Linked population and recreational demand model

![](https://github.com/NOAA-EDAB/presentations/raw/master/docs/EDAB_images/keepOneNoOFkey.png)

*  Increase in angler welfare attainable with same stock status
]
???
...
---
## Updating the risk assessmment: add recreational and cross-sectoral risks, static --> dynamic indicators
...
mafmcsurvEPUcond <- survEPUcond |>
  dplyr::filter(Var %in% mafmc_cond[2],
                EPU %in% c("MAB"))
...
p <-   ggplot2::ggplot() +
  ...
  ggplot2::ggtitle(paste(unique(mafmcsurvEPUcond$Var))) +
...
---
# Evolution of ecosystem reporting
