...
```{r MAFMCriskpolicy, fig.height=4}
...
ggplot2::ggplot(riskpol, aes(x=Bfrac, y=value, linetype = pstar)) +
...
ABCprop <- data.frame(Bfrac = seq(0, 2.5, 0.01)) %>%
  dplyr::mutate(pstar = case_when(Bfrac <= 0.1 ~ 0,
                                  ...
                                  Bfrac > 1.5 ~ 0.49)) %>%
  dplyr::mutate(CV60 = qlnorm(pstar, log(1), log(0.60 + 1)),
                CV100 = qlnorm(pstar, log(1), log(1.00 + 1)),
                CV150 = qlnorm(pstar, log(1), log(1.50 + 1))) %>%
  pivot_longer(-c(Bfrac, pstar), names_to = "ABCprop")
                       

ggplot2::ggplot(ABCprop, aes(x=Bfrac, value, colour=ABCprop)) +
  geom_line()+  
  theme_minimal() +
  xlab("B/Bmsy") +
  ylab("ABC/OFL proportion") +
  labs(colour = "OFL Uncertainty")
