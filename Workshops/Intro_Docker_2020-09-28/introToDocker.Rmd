...
* Max - addressed our need for Docker with the network team
...
`docker run <image name>` - run container from an image
...
`docker rm`  - remove stopped or exited container

`docker images` - lists images

`docker rmi <image name>` - remove image . must reomove all running containers based off image first

`docker pull <image name>` - pull image but doesn't run


ctrl+C  - exits running container

`docker run -d <image name>` - runs container in background. (docker ps to show list of running containers)

`docker logs <container ID>`

`docker attach <containerID>` - to reattach to container
