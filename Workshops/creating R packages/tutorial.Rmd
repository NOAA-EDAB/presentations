...
* Allow you to easily share your code (eg. through a repository on GitHub)
* Great options for code documentation and package documentation (roxygen, rmarkdown)
* Better understanding of how R works
* Integrates with other important/interesting components (mentioned above)
...
#### _devtools_ & _remotes_ & _usethis_{-}
A key package you will need (if you dont have it already) is `devtools`. `devtools` is a package to aid in development of packages. To check to see if you have it, type the following line:
```{r , eval = FALSE}
 "devtools" %in% row.names(installed.packages())
```
A result of FALSE means you dont have it. Install it 
```
install.packages("devtools")
```
Another package you'll need is `remotes`. This allows you to install packages from github (Most of this code used to be in `devtools`)
```
...
Windows: [Rtools](https//cran.r-project.org/bin/windows/Rtools);  Mac: XCode (free in app store); Linux: install R development tools.
...
* Rmarkdown package (for vignette documentation)
* Knitr package (to export rmarkdown)
...
An R package is a collection of functions (not just R code) and data with documentation.

Packages developed by others need to be installed. Often these are installed from either CRAN or Github.

* To install from CRAN `install.packages("packageName")`

* To install from github `remotes::install_github("packageName")`

* To install vignettes associated with package from github

```
remotes::install_github("packageName",build_vignettes = TRUE)
```

The whole package is then loaded into memory using 
```
library(packageName)
```
Individual fuctions from a package can be loaded without having to load the entire library. This becomes important when writing your own packages. Use the syntax
```
packageName::functionName
```

Your own package will be organized in a way akin to folders on your computer. You might keep R code in one folder, data in another, documentation in another etc.
...
All packages have a specific structure that needs to be adhered to. We will use RStudios GUI to create a package (File-> New Project -> R package).

This creates the basic elements for a package (5 components)

1.  A `DESCRIPTION` file - Basic information about the package, authors, contributors, version, title, description, the type of license, and the package dependencies (does it require other packages to work!)
2.  A `NAMESPACE` file - defines the list of functions in your package available to users. It also defines external functions and packages that are imported. (This is automatically generated if using `roxygen2` for documentation so dont worry about this.).
3.  An `R/` directory where your R code will reside
4.  A `man/` directory where documentation will reside
5.  An `.rbuildignore` file where you can specify which files you do not want to include in the package
...
If you have existing code you'd like to turn into a package use:

```
usethis::create_package()
```

This will create all of the same content with the exception of the list item #4 (the man folder used for documentation).
To include this:
```
devtools::document()
```
...
* The documentation resides in the same file as your code. It is then translated to the format R requires (`.Rd` files)
...
#'
#' \item{res1 returned}{description of res1}
#' \item{res2 returned}{description of res2}
#'
#' @section sectionHheadingName:
...
You may create a package that requires data to function. You may not want to make this data available to a user. You can use `usethis::use_data(..., internal=TRUE)` to create a file (in your project root directory) called `sysdata.rda`. This will not get exported. No need to document.


#### _Processed (Raw) data_ {-#raw}
...
To access the package details by using `package?packageName` then a file called packageName.R needs to be created with a couple of additional tags
...
## Sharing your code {-#sharing}

At some point in your package development you should start using git (locally in RStudio) to track ongoing changes to your project/package. Eventually linking to Github so you can share your package with others. Even if you dont plan to share it is still a good option for keeping all of your packages. 
