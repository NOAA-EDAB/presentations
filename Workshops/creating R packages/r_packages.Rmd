...
Three key packages you will need (if you dont have them already) are `devtools`,`remotes`, and `usethis`. `usethis` is a package to aid in development of packages. To check to see if you have it, type the following line:
```{r , eval = FALSE}
...
```
The `devtools` package was split into `remotes` and `usethis`. However there are still a few cases in which you will need `devtools`. It is advisable to install.
```
...
1.  A `DESCRIPTION` file - Basic information about the package, authors, contributors, version, title, description, the type of license, and the package dependencies (other packages required for yours to work!)
